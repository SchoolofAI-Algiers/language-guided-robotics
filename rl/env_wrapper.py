...
class LanguageConditionedWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        """
        if 'pixels' in obs:
            raw_pixels = obs['pixels']
        else:
            raw_pixels = np.zeros((480, 640, 3), dtype=np.uint8)           # shape: (H, W, 3) expected from PyBullet
        if 'object_state' in obs:
            object_state = obs['object_state']
        else:
            object_state = np.zeros(9, dtype=np.float32)   # shape: (9,) provided as mock zero or true query by phase 2

...
class MultimodalObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env, obs_mode="visual_joints_statepybullet"):
        ...
        # We assume the base environment is 'state' (22-dim) or 'pixels' (Box space of RGB image)
        ...
        state_low = np.full(22, -1.0, dtype=np.float32) # Arbitrary bounded proxy
        state_high = np.full(22, 1.0, dtype=np.float32)
        ...
        if hasattr(env, 'observation_space'):
            ...
            elif isinstance(env.observation_space, spaces.Box) and env.observation_space.shape == (22,):

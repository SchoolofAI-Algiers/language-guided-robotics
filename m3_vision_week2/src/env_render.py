...
def render_scene():
    ...
    rgb   = np.reshape(np.array(rgb_raw, dtype=np.uint8), (HEIGHT, WIDTH, 4))[:, :, :3]
    depth = np.reshape(np.array(depth_raw, dtype=np.float32), (HEIGHT, WIDTH))
    seg   = np.reshape(np.array(seg_raw, dtype=np.int32), (HEIGHT, WIDTH))
